...
public class MyOrderAdapter extends RecyclerView.Adapter<MyOrderAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyOrderAdapter.ViewHolder holder, int position) {
        ...
        String orderStatus = myOrderItemModelList.get(position).getDelveryStatus();
        Date date;
        switch (orderStatus) {
            case "Ordered":
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull final View itemView) {
            ...
            deliveryStatus = itemView.findViewById(R.id.delivery_date);
        ...
        private void setData(String resource, String title, String orderStatus, Date date, final int rating, final String productId, final int position) {
            ...
            if (orderStatus.equals("Cancelled")) {
                orderIndicator.setImageTintList(ColorStateList.valueOf(itemView.getContext().getResources().getColor(R.color.BtnBlue)));
            } else {
                                ...
                                FirebaseFirestore.getInstance().collection("USERS").document(FirebaseAuth.getInstance().getUid()).collection("USER_DATA").document("MY_RATINGS")
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            if (DBquerries.myRatedIds.contains(productId)) {
                                                DBquerries.myRating.set(DBquerries.myRatedIds.indexOf(productId), Long.parseLong(String.valueOf(starPosition + 1)));
                                            } else {
