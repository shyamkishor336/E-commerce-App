...
//////cancel order dialog
        ...
        O_P_progress = findViewById(R.id.order_packed_progress_bar);
        ...
        cancelOrderBtn = findViewById(R.id.cancel_btnn);
            ...
            case "Cancelled":
                if (model.getPackedDate().after(model.getOrderedDate())) {
                    if (model.getShippedDate().after(model.getPackedDate())) {
                        ...
                        deliveredIndicator.setImageTintList(ColorStateList.valueOf(getResources().getColor(R.color.BtnBlue)));
                        ...
                        shippedIndicator.setImageTintList(ColorStateList.valueOf(getResources().getColor(R.color.BtnBlue)));
                    ...
                    packedIndicator.setImageTintList(ColorStateList.valueOf(getResources().getColor(R.color.BtnBlue)));
...
//////////rating layout
                            ...
                            FirebaseFirestore.getInstance().collection("USERS").document(FirebaseAuth.getInstance().getUid()).collection("USER_DATA").document("MY_RATINGS")
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        ...
                                        if (DBquerries.myRatedIds.contains(model.getProductId())) {
                                            DBquerries.myRating.set(DBquerries.myRatedIds.indexOf(model.getProductId()), Long.parseLong(String.valueOf(starPosition + 1)));
                                        } else {
                                            DBquerries.myRatedIds.add(model.getProductId());
                                            DBquerries.myRating.add(Long.parseLong(String.valueOf(starPosition + 1)));
                                        }
                                    } else {
        ...
        if (model.isCancellationRequested()) {
            ...
            cancelOrderBtn.setTextColor(getResources().getColor(R.color.BtnBlue));
            cancelOrderBtn.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#ffffff")));
        } else {
            if (model.getDelveryStatus().equals("Ordered") || model.getDelveryStatus().equals("Packed")) {
                cancelOrderBtn.setVisibility(View.VISIBLE);
